...
runMulttest <- function(pvals){
  ...
  secondCol <- rep(NA, nrow(pvals))
  secondCol[(!is.na(pvals))&(pvals==1)] <- 1
  results <- cbind(firstCol,secondCol)
  colnames(results) <- c("Bonferroni","BH")
  filt_pvals <- pvals[(!is.na(pvals))&(pvals!=1)]
  adjPObj <- mt.rawp2adjp(filt_pvals, proc=c("Bonferroni", "BH"))
  sorted <- adjPObj$adjp[order(adjPObj$index),-1]
  results[((!is.na(pvals))&(pvals!=1))] <- sorted
  results
}
...
ppdeMix <- function(pVal,n){
  ...
  if (ncol(pVal)==2){
    Pdata <- pVal[,2]
    Pdata <- Pdata[(!is.na(Pdata))&(Pdata!=1)] }
  else{
    Pdata <- pVal[(!is.na(pVal))&(pVal!=1)] }
  Pdata<-adjPVal(Pdata)
  ...
  if(ncol(pVal)==2){
    postP1[(!is.na(pVal[,2]))&(pVal[,2]==1)] <- 0
    postP2[(!is.na(pVal[,2]))&(pVal[,2]==1)] <- 0
    rocX[(!is.na(pVal[,2]))&(pVal[,2]==1)] <- 1
    rocY[(!is.na(pVal[,2]))&(pVal[,2]==1)] <- 1 }
  else{
    postP1[(!is.na(pVal))&(pVal==1)] <- 0
    postP2[(!is.na(pVal))&(pVal==1)] <- 0
    rocX[(!is.na(pVal))&(pVal==1)] <- 1
    rocY[(!is.na(pVal))&(pVal==1)] <- 1 }
  ...
  if(n==1){
    ...
    denom2 <- numer2 + L0
    if (ncol(pVal)==2){
      rocX[(!is.na(pVal[,2]))&(pVal[,2]!=1)]<-Pdata
      rocY[(!is.na(pVal[,2]))&(pVal[,2]!=1)]<-pbeta(Pdata,r1,s1) }
    else{
      rocX[(!is.na(pVal))&(pVal!=1)]<-Pdata
      rocY[(!is.na(pVal))&(pVal!=1)]<-pbeta(Pdata,r1,s1) } }
    ...
    if (ncol(pVal)==2){
      rocX[(!is.na(pVal[,2]))&(pVal[,2]!=1)]<-Pdata
      rocY[(!is.na(pVal[,2]))&(pVal[,2]!=1)]<-(L1*pbeta(Pdata,r1,s1) + L2 * pbeta(Pdata,r2,s2))/(L1+L2) }
    else{
      rocX[(!is.na(pVal))&(pVal!=1)]<-Pdata
      rocY[(!is.na(pVal))&(pVal!=1)]<-(L1*pbeta(Pdata,r1,s1) + L2 * pbeta(Pdata,r2,s2))/(L1+L2) } }
    ...
    if (ncol(pVal)==2){
      rocX[(!is.na(pVal[,2]))&(pVal[,2]!=1)]<-Pdata
      rocY[(!is.na(pVal[,2]))&(pVal[,2]!=1)]<-(L1*pbeta(Pdata,r1,s1) + L2 * pbeta(Pdata,r2,s2) + L3 * 
                               pbeta(Pdata,r3,s3))/(L1+L2+L3) }
    else{
      rocX[(!is.na(pVal))&(pVal!=1)]<-Pdata
      rocY[(!is.na(pVal))&(pVal!=1)]<-(L1*pbeta(Pdata,r1,s1) + L2 * pbeta(Pdata,r2,s2) + L3 * 
                           pbeta(Pdata,r3,s3))/(L1+L2+L3) } }
  else cat("Error: Must fit 1,2, or 3 beta's (change betaFit)\n")
  
  if (ncol(pVal)==2){
    postP1[(!is.na(pVal[,2]))&(pVal[,2]!=1)] <- numer1/denom1
    postP2[(!is.na(pVal[,2]))&(pVal[,2]!=1)] <- numer2/denom2 }
  else{
    postP1[(!is.na(pVal))&(pVal!=1)] <- numer1/denom1
    postP2[(!is.na(pVal))&(pVal!=1)] <- numer2/denom2 }
